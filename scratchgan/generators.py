...
class LSTMGen(snt.AbstractModule):
  ...
  """
    ...
    def _build(self, is_training=True, temperature=1.0):
        input_keep_prob = (1.0 - self._input_dropout) if is_training else 1.0
        output_keep_prob = (1.0 - self._output_dropout) if is_training else 1.0
        ...
        output_embeddings = tf.nn.dropout(all_embeddings, keep_prob=output_keep_prob)
        ...
        if len(self._feature_sizes) > 1:
            out_proj = tf.get_variable(
                "out_proj", shape=[self._embedding_size, sum(self._feature_sizes)]
            )
        else:
            out_proj = in_proj
        ...
        for _ in xrange(max_sequence_length):
            ...
            logits = (
                tf.matmul(outputs_proj, output_embeddings, transpose_b=True) + out_bias
            )
        ...
        embeddings = tf.stack(  # pylint: disable=unused-variable
            embeddings_list, axis=1, name="embeddings"
        )
        ...
        sequence_length = utils.get_first_occurrence_indices(sequence, self._pad_token)
