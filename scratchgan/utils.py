...
def get_position_signal(sequence_length, position_dim=8):
  ...
  """
    ...
    periods = tf.exp(
        tf.lin_space(tf.log(2.0), tf.log(tf.to_float(sequence_length)), position_dim)
    )
...
def get_mask_past_symbol(reference, symbol, optimize_for_tpu=False):
    ...
    first_indices = get_first_occurrence_indices(reference, symbol, optimize_for_tpu)
...
def get_first_occurrence_indices(reference, symbol, optimize_for_tpu=False):
  ...
  """
    ...
    index_first_occurrences = tf.segment_min(
        index_all_occurrences[:, 1], index_all_occurrences[:, 0]
    )
...
def write_eval_results(
    ...
    to_write = ",".join(
        map(
            str, [checkpoint_name, mean_train_prob, mean_valid_prob, mean_gen_prob, fid]
        )
    )
    ...
    with gfile.GFile(
        os.path.join(checkpoint_dir, checkpoint_name + "_sentences.txt"), "w"
    ) as f:
...
def maybe_pick_models_to_evaluate(checkpoint_dir):
    ...
    checkpoints_to_evaluate = [f for f in filenames if f not in evaluated_filenames]
...
def make_partially_trainable_embeddings(
    vocab_file, embedding_source, vocab_size, trainable_embedding_size
):
  ...
  """
    ...
    embedding_initializer = _get_embedding_initializer(
        vocab_file=vocab_file, embedding_source=embedding_source, vocab_size=vocab_size
    )
    pretrained_embedding = tf.get_variable(
        "pretrained_embedding", initializer=embedding_initializer, dtype=tf.float32
    )
