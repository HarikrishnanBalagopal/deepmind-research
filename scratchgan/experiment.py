...
flags.DEFINE_integer("trainable_embedding_size", 64, "Size of trainable embedding.")
...
flags.DEFINE_string(
    "checkpoint_dir", "/tmp/emnlp2017/checkpoints/", "Directory for checkpoints."
)
flags.DEFINE_integer("export_every", 1000, "Frequency of checkpoint exports.")
flags.DEFINE_integer(
    "num_examples_for_eval", int(1e4), "Number of examples for evaluation"
)
...
def train(config):
    ...
    iterator_valid = reader.iterator(raw_data=valid_data, batch_size=config.batch_size)

    real_sequence = tf.placeholder(
        dtype=tf.int32, shape=[config.batch_size, max_length], name="real_sequence"
    )
    ...
    test_fake_batch = {
        "sequence": tf.constant(
            np.random.choice(vocab_size, size=[config.batch_size, max_length]).astype(
                np.int32
            )
        ),
        "sequence_length": tf.constant(
            np.random.choice(max_length, size=[config.batch_size]).astype(np.int32)
        ),
    }
    ...
    gen = generators.LSTMGen(
        ...
        pad_token=(word_to_id["<PAD>"] if config.dataset == "quoratext" or config.dataset == "emnlp2017" else reader.PAD_INT),
    ...
    disc_logits_fake = disc(
        sequence=gen_outputs["sequence"], sequence_length=gen_outputs["sequence_length"]
    )
    ...
    if config.disc_loss_type == "ce":
        ...
        loss_real = losses.sequential_cross_entropy_loss(disc_logits_real, targets_real)
        loss_fake = losses.sequential_cross_entropy_loss(disc_logits_fake, targets_fake)
    ...
    valid_disc_predictions = tf.reduce_mean(tf.nn.sigmoid(valid_disc_logits), axis=0)
    ...
    with tf.Session() as sess:
        ...
        logging.info(
            "Saved final model at %s.",
            tf.train.latest_checkpoint(config.checkpoint_dir),
        )


def evaluate_pair(
    config, batch_size, checkpoint_path, data_dir, dataset, num_examples_for_eval
):
  ...
  """
    ...
    # Build graph.
    train_data, valid_data, word_to_id = reader.get_raw_data(data_dir, dataset=dataset)
    id_to_word = {v: k for k, v in word_to_id.iteritems()}
    vocab_size = len(word_to_id)
