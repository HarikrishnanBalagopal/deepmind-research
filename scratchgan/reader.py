...
def _integerize(json_data, word_to_id, dataset):
    ...
    sequences = np.full(
        (len(json_data), MAX_TOKENS_SEQUENCE[dataset]), word_to_id[PAD], np.int32
    )
    sequence_lengths = np.zeros(shape=(len(json_data)), dtype=np.int32)
    for i, sentence in enumerate(json_data):
        sequence_i = string_sequence_to_sequence(tokenize(sentence["s"]), word_to_id)
...
def get_raw_data(data_path, dataset, truncate_vocab=20000):
  ...
  """
    if dataset not in FILENAMES:
        raise ValueError(
            "Invalid dataset {}. Valid datasets: {}".format(dataset, FILENAMES.keys())
        )
    ...
    word_to_id_truncated = {k: v for k, v in word_to_id.items() if v < truncate_vocab}
