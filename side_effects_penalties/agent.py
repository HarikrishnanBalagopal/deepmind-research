...
class EpsilonGreedyPolicy(object):
    ...
    """
        if np.random.random() < epsilon:
            ...
            values = [self._value_function[(state, action)] for action in self._actions]
...
class QLearning(object):
    ...
    def __init__(
        self, actions, alpha=0.1, epsilon=0.1, q_initialisation=0.0, discount=0.99
    ):
    ...
    """
        ...
        self._policy = EpsilonGreedyPolicy(self._value_function, self._valid_actions)
    ...
    def _update(self, timestep, state):
        ...
        # Non-terminal state.
            ...
            delta = (
                reward
                + self.discount * self._value_function[(state, max_action)]
                - self._value_function[(self._current_state, self._current_action)]
            )
