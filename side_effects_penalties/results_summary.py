...
if __name__ == "__main__":  # Avoid defining flags when used as a library.
    ...
    flags.DEFINE_string(
        "input_suffix", "", "Filename suffix to use when loading data files."
    )
    flags.DEFINE_string(
        "output_suffix", "", "Filename suffix to use when saving files."
    )
    flags.DEFINE_bool(
        "bar_plot",
        True,
        "Make a data frame for a bar plot (True) " + "or learning curves (False)",
    )
    flags.DEFINE_string("env_name", "box", "Environment name.")
    flags.DEFINE_bool("noops", True, "Whether the environment includes noops.")
    flags.DEFINE_list(
        "beta_list", [0.1, 0.3, 1.0, 3.0, 10.0, 30.0, 100.0], "List of beta values."
    )
...
def make_summary_data_frame(
    ...
    if compare_penalties:
        filename = (
            "df_summary_penalties_" + env_name + final_str + output_suffix + ".csv"
        )
    else:
        filename = (
            "df_summary_betas_"
...
def main(unused_argv):
    ...
    make_summary_data_frame(
        ...
        noops=FLAGS.noops,
        final=FLAGS.bar_plot,
        dev_measure=dev_measure,
        value_discount=value_discount,
        dev_fun=dev_fun,
        path=FLAGS.path,
        input_suffix=FLAGS.input_suffix,
        output_suffix=FLAGS.output_suffix,
        beta_list=FLAGS.beta_list,
        seed_list=FLAGS.seed_list,
    )
