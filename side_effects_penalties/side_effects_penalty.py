...
class Baseline(object):
    ...
    def __init__(self, start_timestep, exact=False, env=None, timestep_to_state=None):
        """Create a baseline.
...
class InactionBaseline(Baseline):
    ...
    def calculate(self, prev_state, action, current_state):
        if self._exact:
            self._baseline_state = self._timestep_to_state(self._env.step(Actions.NOOP))
        else:
...
class StepwiseBaseline(Baseline):
    ...
    def parallel_inaction_rollouts(self, s1, s2):
        ...
        while states not in chain:
            ...
            s1, s2 = states
            states = (
                self.sample(s1) if s1 in self._inaction_next else s1,
                    ...
                    for s2 in self._reachability[current_state].keys():
                        if self._reachability[current_state][s2] > 0:
                            self._reachability[s1][s2] = max(
                                ...
                                self._reachability[s1][prev_state]
                                * self._value_discount
...
class AttainableUtilityMixin(object):
  ...
  """
    ...
    def calculate(self, current_state, baseline_state, rollout_func=None):
        ...
        penalties = [
            self._dev_fun(base_val - cur_val) * (1.0 - self._value_discount)
    ...
    """
        ...
        for st in chain:
            rollout_values = [
                rv + coeff * u_val[st] * (1.0 - self._discount)
                for rv, u_val in zip(rollout_values, self._u_values)
            ]
            coeff *= self._discount
        last_state = chain[-1]
        rollout_values = [
            rv + coeff * u_val[last_state]
            for rv, u_val in zip(rollout_values, self._u_values)
        ]
        return rollout_values
    ...
    """
        if isinstance(util_funs, int):
            self._util_funs = [collections.defaultdict(float) for _ in range(util_funs)]
        else:
...
class AttainableUtility(AttainableUtilityMixin, DeviationMeasure):
    ...
    def __init__(
        self, value_discount=0.99, dev_fun=np.abs, util_funs=10, discount=None
    ):
...
class SideEffectPenalty(object):
    ...
    def __init__(self, baseline, dev_measure, beta=1.0, use_inseparable_rollout=False):
    ...
    def calculate(self, prev_state, action, current_state):
        ...
        if current_state:
            ...
            baseline_state = self._baseline.calculate(prev_state, action, current_state)
            if self._use_inseparable_rollout:
                penalty = self._rollout_value(
                    current_state,
                    baseline_state,
                ...
                penalty = self._dev_measure.calculate(
                    current_state, baseline_state, self._baseline.rollout_func
                )
