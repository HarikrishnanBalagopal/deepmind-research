...
class CS(object):
    ...
    def __init__(
        self, metric_net, generator, num_z_iters, z_step_size, z_project_method
    ):
    ...
    """
        ...
        optimisation_cost = utils.get_optimisation_cost(generator_inputs, optimised_z)
        debug_ops = {}
        ...
        debug_ops["recons_loss"] = tf.reduce_mean(
            tf.norm(snt.BatchFlatten()(samples) - snt.BatchFlatten()(data), axis=-1)
        )
    ...
    """
        ...
        img_diff_norm = tf.norm(
            snt.BatchFlatten()(img1) - snt.BatchFlatten()(img2), axis=-1
        )
