...
class ModelOutputs(
    collections.namedtuple(
        "AdversarialModelOutputs", ["optimization_components", "debug_ops"]
    )
):
...
def cross_entropy_loss(logits, expected):
  ...
  """
    ...
    if num_logits != 2:
        raise ValueError(
            (
                "Invalid number of logits for cross_entropy_loss! "
                "cross_entropy_loss supports only 2 output logits!"
            )
        )
    return tf.reduce_mean(
        tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=expected)
    )
