...
flags.DEFINE_integer(
    "num_training_iterations", 200000, "Number of training iterations."
)
flags.DEFINE_integer("batch_size", 64, "Training batch size.")
flags.DEFINE_integer("num_latents", 128, "The number of latents")
flags.DEFINE_integer(
    "summary_every_step", 1000, "The interval at which to log debug ops."
)
flags.DEFINE_integer(
    "image_metrics_every_step",
    2000,
    "The interval at which to log (expensive) image metrics.",
)
flags.DEFINE_integer("export_every", 10, "The interval at which to export samples.")
flags.DEFINE_integer(
    "num_eval_samples", 10000, "The number of samples used to evaluate FID/IS"
)
flags.DEFINE_string("dataset", "cifar", "The dataset used for learning (cifar|mnist.")
flags.DEFINE_float(
    "optimisation_cost_weight", 3.0, "weight for latent optimisation cost."
)
flags.DEFINE_integer(
    "num_z_iters",
    3,
    "The number of latent optimisation steps."
    "It falls back to vanilla GAN when num_z_iters is set to 0.",
)
flags.DEFINE_float("z_step_size", 0.01, "Step size for latent optimisation.")
flags.DEFINE_string("z_project_method", "norm", "The method to project z.")
flags.DEFINE_string(
...
flags.DEFINE_bool(
    "run_real_data_metrics", False, "Whether or not to run image metrics on real data."
)
flags.DEFINE_bool(
    "run_sample_metrics", True, "Whether or not to run image metrics on samples."
)
...
def main(argv):
    ...
    images = utils.get_train_dataset(data_processor, FLAGS.dataset, FLAGS.batch_size)
    ...
    eval_real_data = utils.get_real_data_for_eval(
        FLAGS.num_eval_samples, FLAGS.dataset, split="train"
    )

    def sample_fn(x):
        return utils.optimise_and_sample(x, module=model, data=None, is_training=False)[
            0
        ]
    ...
    sample_exporter = file_utils.FileExporter(os.path.join(FLAGS.output_dir, "samples"))
