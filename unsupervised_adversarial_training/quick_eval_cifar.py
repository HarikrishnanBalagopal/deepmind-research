...
FLAGS = flags.FLAGS
flags.DEFINE_enum(
    "attack_fn_name", "fgsm", ["fgsm", "none"], "Name of the attack method to use."
)
flags.DEFINE_float(
    "epsilon_attack", 8.0 / 255, "Maximum allowable perturbation size, between 0 and 1."
)
...
def _build_dataset(raw_data, batch_size=32, shuffle=False):
  ...
  """
    ...
    samples = Sample(images.astype(np.float32) / 255.0, labels.astype(np.int64))
...
def _cifar_meanstd_normalize(image):
  ...
  """
    ...
    rescaled_means = [x / 255.0 for x in cifar_means]
    rescaled_devs = [x / 255.0 for x in cifar_devs]
