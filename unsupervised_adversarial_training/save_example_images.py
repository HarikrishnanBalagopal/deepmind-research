...
FLAGS = flags.FLAGS
flags.DEFINE_string("data_bin_path", None, "path to 80M Tiny Images data binary")
flags.DEFINE_string(
    "idxs_path",
    os.path.join(DIR_NAME, "tiny_200K_idxs.txt"),
    "path to file of indices indicating subset of 80M dataset",
)
flags.DEFINE_string(
    "output_dir",
    os.path.join(DIR_NAME, "images"),
    "path to output directory for images",
)
flags.mark_flag_as_required("data_bin_path")

CIFAR_LABEL_IDX_TO_NAME = [
    "airplane",
    "automobile",
    "bird",
    "cat",
...
def _load_dataset_as_array(ds_path):
    dataset = np.memmap(
        filename=ds_path, dtype=np.uint8, mode="r", shape=(DATASET_SIZE, 3, 32, 32)
    )
