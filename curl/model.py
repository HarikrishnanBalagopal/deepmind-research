    ...
    """
        ...
        if encoder_type == "conv":
            self.shared_encoder = layers.SharedConvModule(
                filters=n_enc, strides=enc_strides, kernel_size=3, activation=tf.nn.relu
            )
...
def data_decoder_fn(
  ...
  """
    ...
    if len(z.shape) != 2:
        raise NotImplementedError(
            "The data decoder function expects `z` to be "
            "2D, but its shape was %s instead." % str(z.shape)
        )
    if len(y.shape) != 2:
        raise NotImplementedError(
            "The data decoder function expects `y` to be "
...
def latent_decoder_fn(y, n_z, is_training=True):
  ...
  """
    ...
    if len(y.shape) != 2:
        raise NotImplementedError(
            "The latent decoder function expects `y` to be "
            "2D, but its shape was %s instead." % str(y.shape)
        )
...
class Curl(object):
    ...
    """
        ...
        with tf.name_scope("{}_log_prob_elbo".format(self.scope_name)):
            ...
            log_p_x_sup = tf.einsum(
                "ij,jik->ik", self.y_label[:, : self._n_y_active], log_p_x_all
            )  # [B, I]
            kl_z_sup = tf.einsum(
                "ij,ij->i", self.y_label[:, : self._n_y_active], kl_z_all
            )  # [B]
            # -log q(y=y_true | x)
            ...
            self.log_p_x_all = tf.transpose(
                reduce_op(log_p_x_all, -1, name="log_p_x_all")  # [B, n_y]
            )
            self.kl_z_all = kl_z_all
            self.y_probs = y_probs

        self._cache[cache_key] = (log_p_x, kl_y, kl_z, log_p_x_sup, kl_y_sup, kl_z_sup)
    ...
    """
        ...
        try:
            ...
            if self._kly_over_batch:
                probs = tf.reduce_mean(q.probs, axis=0, keepdims=True) * tf.ones_like(
                    q.probs
                )
        ...
        if use_mean_y:
            ...
            z_all = tf.map_fn(
                fn=lambda y: self._latent_encoder(
                    hiddens, y, is_training=self._is_training
                ).mean(),
                elems=all_y,
                dtype=tf.float32,
            )
  ...
  """
    ...
    def __init__(
        ...
        assert len(filters) == len(dec_up_strides) + 1, (
            ...
            % (len(filters), len(dec_up_strides) + 1, str(filters), str(dec_up_strides))
        )
    ...
    def _build(self, z, is_training=True, test_local_stats=True, use_bn=False):
        ...
        if use_bn:
            upsample_mlp_flat = snt.BatchNorm(scale=True)(
                upsample_mlp_flat, **batch_norm_args
            )
