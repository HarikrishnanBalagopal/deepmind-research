...
MainOps = collections.namedtuple(
    "MainOps",
    [
        "elbo",
        "ll",
...
def get_data_sources(
  ...
  """
    ...
    if training_data_type == "sequential":
        ...
        if n_concurrent_classes == 1:
            ...
            filter_fn = lambda v: tf.logical_and(
                tf.greater_equal(v[label_key], cmin[c]), tf.less(v[label_key], cmax[c])
            )
    ...
    full_ds = full_ds.map(preprocess_data)
    train_datasets_for_classifier = full_ds.batch(test_batch_size, drop_remainder=True)
    ...
    try:
        ...
        assert num_valid_examples % test_batch_size == 0, (
            "test_batch_size must be a divisor of %d" % num_valid_examples
        )
    ...
    # Load test dataset.
    test_dataset = tfds.load(name=dataset, split=tfds.Split.TEST, **dataset_kwargs)
    num_test_examples = ds_info.splits["test"].num_examples
    assert num_test_examples % test_batch_size == 0, (
        "test_batch_size must be a divisor of %d" % num_test_examples
    )
...
def setup_training_and_eval_graphs(
    x, label, y, n_y, curl_model, classify_with_samples, is_training, name
):
  ...
  """
    ...
    (
        ...
        kl_y_supervised,
        kl_z_supervised,
    ) = curl_model.log_prob_elbo_components(x, y)
    ...
    confusion = tf.confusion_matrix(
        label, tf.argmax(cat_probs, axis=1), num_classes=n_y, name=name + "_confusion"
    )
    purity = tf.reduce_sum(tf.reduce_max(confusion, axis=0)) / tf.reduce_sum(confusion)

    if classify_with_samples:
        latents = curl_model.infer_latent(
            hiddens=hiddens, y=tf.to_float(cat.sample())
        ).sample()
    else:
        latents = curl_model.infer_latent(
            hiddens=hiddens, y=tf.to_float(cat.mode())
        ).mean()
...
def get_generated_data(sess, gen_op, y_input, gen_buffer_size, component_counts):
  ...
  """
    ...
    for _ in range(gen_buffer_size):
        gen_label = np.random.choice(
            np.arange(n_y), size=(batch_size,), replace=True, p=cluster_sample_probs
        )
...
def setup_dynamic_ops(n_y):
  ...
  """
    ...
    # Create update ops
    cluster_w_update_op = tf.scatter_nd_update(cluster_w, w_indices, cluster_w_updates)
    cluster_b_update_op = tf.scatter_update(cluster_b, b_indices, cluster_b_updates)

    # 3) Ops to get and set latent prior params (columns of a tensor)
    # We will be copying column ind_from to column ind_to.
    latent_prior_mu_w = graph.get_tensor_by_name("latent_decoder/latent_prior_mu/w:0")
    ...
    mu_updates = tf.squeeze(
        tf.slice(
            latent_prior_mu_w, begin=(ind_from, 0), size=(1, latent_prior_mu_w.shape[1])
        )
    )
    ...
    sigma_update_op = tf.scatter_nd_update(
        latent_prior_sigma_w, sigma_indices, sigma_updates
    )
...
def run_training(
  ...
  """
    ...
    if gen_replay_type == "fixed":
        data_period = data_period = int(n_steps / (n_classes / n_concurrent_classes))
    ...
    x_train_raw = tfc.placeholder(dtype=tf.float32, shape=(batch_size,) + output_shape)
    ...
    prior_test_probs = utils.construct_prior_probs(test_batch_size, n_y, n_y_active)
    ...
    hiddens_for_clf = model_eval.get_shared_rep(x_train_for_clf, is_training=False)
    cat_for_clf = model_eval.infer_cluster(hiddens_for_clf)

    if classify_with_samples:
        latents_for_clf = model_eval.infer_latent(
            hiddens=hiddens_for_clf, y=tf.to_float(cat_for_clf.sample())
        ).sample()
    else:
        latents_for_clf = model_eval.infer_latent(
            hiddens=hiddens_for_clf, y=tf.to_float(cat_for_clf.mode())
        ).mean()
    ...
    lr_schedule = [
        tf.cast(el * num_train_examples / batch_size, tf.int64) for el in lr_schedule
    ]
    ...
    if gen_every_n > 0:
        ...
        gen_buffer_size = min(int(gen_refresh_period / gen_every_n), max_gen_batches)
    ...
    if train_supervised:
        ...
        to_log += [
            "train_ELBO_supervised",
            "train_log_p_x_supervised",
            "train_kl_y_supervised",
            "train_kl_z_supervised",
        ]
    else:
        ...
        for step in range(n_steps):
            ...
            if (
                ...
                if training_data_type == "sequential":
                    ...
                    if blend_classes:
                        ...
                        if (
                            step % n_steps_per_class < n_steps_per_class / 4
                            and current_data_period > 0
                        ):
                            ...
                            num_other = int(
                                (n_steps_per_class / 2 - 2 * (step % n_steps_per_class))
                                * batch_size
                                / n_steps_per_class
                            )
                            other_inds = np.random.permutation(batch_size)[:num_other]

                            train_data_array[image_key][
                                :num_other
                            ] = other_train_data_array[image_key][other_inds]
                            train_data_array[label_key][
                                :num_other
                            ] = other_train_data_array[label_key][other_inds]

                        # If in the last quarter, blend in examples from the next class
                        elif (
                            step % n_steps_per_class > 3 * n_steps_per_class / 4
                            and current_data_period < n_classes - 1
                        ):
                            other_train_data_array = sess.run(
                                train_data[current_data_period + 1]
                            )
                            ...
                            num_other = int(
                                (
                                    2 * (step % n_steps_per_class)
                                    - 3 * n_steps_per_class / 2
                                )
                                * batch_size
                                / n_steps_per_class
                            )
            ...
            if (step + 1) % report_interval == 0:
                if valid_data is not None:
                    ...
                    proc_ops = {
                        k: (np.sum if "confusion" in k else np.mean)
                        for k in valid_ops_to_run
                    }
                    results.update(
                        process_dataset(
            ...
            if dynamic_expansion and used_real_data:
                ...
                if (
                    steps_since_expansion >= exp_wait_steps
                    and step >= exp_burn_in
                    and n_y_active_np < n_y
                ):
                ...
                if eligible_for_expansion:
                    ...
                    if n_poor_data >= exp_buffer_size:
                        ...
                        poor_data_buffer = poor_data_buffer[
                            -(n_poor_batches * batch_size) :
                        ]
                        poor_data_labels = poor_data_labels[
                            -(n_poor_batches * batch_size) :
                        ]
                        ...
                        for bs in range(n_poor_batches):
                            poor_cprobs.append(
                                sess.run(
                                    train_ops.cat_probs,
                                    feed_dict={
                                        x_train_raw: poor_data_buffer[
                                            bs * batch_size : (bs + 1) * batch_size
                                        ]
                                    },
                                )
                            )
                        ...
                        copy_component_params(
                            best_cluster, new_cluster, sess, **dynamic_ops
                        )
                        ...
                        for _ in range(num_buffer_train_steps):
                            for bs in range(n_poor_batches):
                                x_batch = poor_data_buffer[
                                    bs * batch_size : (bs + 1) * batch_size
                                ]
                                label_batch = poor_data_labels[
                                    bs * batch_size : (bs + 1) * batch_size
                                ]
            ...
            if used_real_data:
                ...
                recent_component_counts += np.sum(train_cat_probs_vals, axis=0).astype(
                    float
                )
            ...
            if (step + 1) % report_interval == 0:
                ...
                clf_test_vals = process_dataset(
                    dataset_ops.test_iter,
                    {"latents": test_ops.latents, "labels": test_data[label_key]},
                    sess,
                    aggregation_ops=np.concatenate,
                )
