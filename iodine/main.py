...
def default_config():
    ...
    checkpoint_dir = (
        "checkpoints/iodine"  # if continue_run is False, "_{run_id}" will be appended
    )
...
def get_train_step(model, dataset, optimizer):
    ...
    with tf.control_dependencies([train_op]):
        ...
        summaries = {k: tf.summary.scalar(k, v) for k, v in scalars.items()}
        summaries.update({k: tf.summary.image(k, v) for k, v in overview.items()})
...
def get_checkpoint_dir(continue_run, checkpoint_dir, _run, _log):
    ...
    _log.info("Starting a new run using checkpoint dir: '{}'".format(checkpoint_dir))
    return checkpoint_dir


@ex.capture
def get_session(
    chkp_dir, loss, stop_after_steps, save_summaries_steps, save_checkpoint_steps
):
...
def main(save_summaries_steps):
    ...
    gstep, train_step_exports, train_op = get_train_step(model, dataset, optimizer)
    ...
    with get_session(checkpoint_dir, train_step_exports["loss/total"]) as sess:
        while not sess.should_stop():
            ...
            if step % save_summaries_steps == 0:
                ...
                print(
                    "{step:>6d} Loss: {loss: >12.2f}\t\tARI-nobg:{ari: >6.2f}".format(
