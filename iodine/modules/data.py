...
class IODINEDataset(snt.AbstractModule):
    ...
    def preprocess(self, data):
        sg = ShapeGuard(
            dims={"B": self.batch_size, "H": self.image_dim[0], "W": self.image_dim[1]}
        )
        ...
        # rescale
        size = tf.constant(self.image_dim, dtype=tf.int32, shape=[2], verify_shape=True)
    ...
    def preprocess_factors(self, data, sg):
        return {
            name: sg.guard(ensure_3d(data[name]), "B, L, *") for name in self.factors
        }
    ...
    def get_placeholders(self, batch_size=None):
        ...
        return {
            ...
            "factors": {
                name: tf.placeholder(dtype=dtype, shape=sg["B, L, {}".format(size)])
...
class CLEVR(IODINEDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            path=path, crop_region=crop_region, image_dim=image_dim, name=name, **kwargs
        )
