...
__all__ = ("get_mask_plot_colors", "example_plot", "iterations_plot", "inputs_plot")
...
def example_plot(rinfo, b=0, t=-1, mask_components=False, size=2, column_titles=True):
...
def iterations_plot(rinfo, b=0, mask_components=False, size=2):
    ...
    for k in range(K):
        ...
        show_mat(pred_mask_logits[T - 1, k], ax=axes[T, k + 2], vmin=vmin, vmax=vmax)
        axes[T, k + 2].set_xlabel(
            "Mask Logits for\nComponent {}".format(k + 1)
        )  # , color=colors[k])
