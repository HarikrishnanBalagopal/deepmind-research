...
def get_distribution(name_or_dist):
    ...
    raise KeyError(
        'Unknown distribution "{}" of type {}"'.format(name_or_dist, type(name_or_dist))
    )
...
def construct_reconstr_image(images, recons, border_width=2, nr_images=6, clip=True):
...
def print_shapes(name, value, indent=""):
    if isinstance(value, dict):
        ...
        for k, v in sorted(value.items(), key=lambda x: (isinstance(x[1], dict), x[0])):
...
def _pad_images(images, image_border_value=0.5, border_width=2):
    ...
    return tf.pad(images, paddings, "CONSTANT", constant_values=image_border_value)
