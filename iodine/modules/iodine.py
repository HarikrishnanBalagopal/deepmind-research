...
DEFAULT_PREPROCESSING = ["dcomponents", "dmask", "dzp", "log_prob", "counterfactual"]

DEFAULT_STOP_GRADIENT = ("dzp", "dmask", "dcomponents", "log_prob", "counterfactual")
...
class IODINE(snt.AbstractModule):
  ...
  """
    ...
    def _build(self, data):
        ...
        total_rec_loss = sum(
            [w * re for w, re in zip(self.iter_loss_weights, iterations["recons_loss"])]
        )
        total_kl_loss = sum(
            [
        ...
        scalars = {
            "loss/kl": sum(
                [tf.reduce_mean(tf.reduce_sum(kl, axis=1)) for kl in iterations["kl"]]
            ),
            "loss/recons": total_rec_loss,
        }

        if self.factor_evaluator:
            pred_mask = self._sg.guard(
                x_dist.mixture_distribution.probs, "B, 1, H, W, K"
            )
    ...
    def eval(self, data):
        ...
        def get_components(dist):
            return tf.transpose(
                dist.components_distribution.mean()[:, 0, :, :, :, :], [0, 3, 1, 2, 4]
            )

        def get_mask(dist):
            return tf.transpose(
                dist.mixture_distribution.probs[:, :, :, :, :], [0, 4, 2, 3, 1]
            )

        def get_mask_logits(dist):
            return tf.transpose(
                dist.mixture_distribution.logits[:, :, :, :, :], [0, 4, 2, 3, 1]
            )
        ...
        inputs_flat = {
            k: stack_iters(
                [inp["flat"][k] for inp in iterations["inputs"]], pad_zero=True
            )
            for k in iterations["inputs"][0]["flat"].keys()
        }
        inputs_spatial = {
            k: stack_iters(
                [inp["spatial"][k] for inp in iterations["inputs"]], pad_zero=True
            )
        ...
        components = stack_iters([get_components(xd) for xd in iterations["x_dist"]])
        ...
        info = {
            ...
            "inputs": {"flat": inputs_flat, "spatial": inputs_spatial},
            "latent": {
            ...
            "metrics": {"ari": ari, "ari_nobg": ari_nobg, "mse": mse},
        }
        ...
        if self.factor_evaluator:
            ...
            info["factor_regressor"] = {
                "assignment": sg.guard(
                    stack_iters(factor_info["assignment"]), "B, T, L, K"
                ),
                "metrics": {
    ...
    def get_overview_images(self, data, nr_images=4, mask_components=False):
        ...
        # iterations view
        recons = tf.stack([x_dist.mean() for x_dist in iterations["x_dist"]], axis=1)
        masks = tf.stack(
            [
        ...
        return {
            "overview": utils.get_overview_image(
                x[:, t : t + 1], x_dist, mask_components=mask_components
            ),
            "sequence": utils.construct_iterations_image(
                x[:, : t + 1, tf.newaxis], recons, masks
            ),
    ...
    def _parse_iter_loss_weights(self, iter_loss_weight):
        if iter_loss_weight == "linspace":
            ...
            raise ValueError(
                "Unknown iter_loss_weight type {}.".format(repr(iter_loss_weight))
            )
    ...
    def _get_inputs_for(self, out_params, out_dist, img, z_dist, zp, loss):
        ...
        for k in range(0, self.num_components):
            mask = tf.concat(
                [out_params.mask[:, :k], out_params.mask[:, k + 1 :]], axis=1
            )
            pixel = tf.concat(
                [out_params.pixel[:, :k], out_params.pixel[:, k + 1 :]], axis=1
            )
        ...
        pred_mask = tf.transpose(out_dist.mixture_distribution.probs, [0, 4, 2, 3, 1])

        potential_inputs = {
            # spatial
            "image": sg.guard(img, "B, 1, H, W, C"),
            "log_prob": sg.guard(log_prob, "B, 1, H, W, 1"),
            "mask": sg.guard(out_params.mask, "B, K, H, W, 1"),
            "pred_mask": sg.guard(pred_mask, "B, K, H, W, 1"),
            "components": sg.guard(out_params.pixel, "B, K, H, W, Cp"),
            "dmask": sg.guard(dmp, "B, K, H, W, Mp"),
            "dcomponents": sg.guard(dxp, "B, K, H, W, Cp"),
            "posterior": sg.guard(
                self._get_mask_posterior(out_dist, img), "B, K, H, W, 1"
            ),
            "capacity": 0.5
            * tf.ones(sg["B, K, H, W, 1"], dtype=tf.float32),  # TODO: legacy
            "coordinates": self._get_coord_channels(),
            "counterfactual": self._sg.guard(counterfactual, "B, K, H, W, 1"),
            # flat
            "zp": sg.guard(zp, "B, K, Zp"),
            "dzp": sg.guard(dzp, "B, K, Zp"),
            "flat_capacity": 0.5
            * tf.ones(sg["B, K, 1"], dtype=tf.float32),  # TODO: legacy
        }
    ...
    def _apply_preprocessing(self, name, val):
        if name in self.preprocess:
            if self._sg.matches(val, "B, K, _z"):
                ...
                raise ValueError(
                    "Cannot handle shape {}".format(val.get_shape().as_list())
                )
    ...
    def _get_coord_channels(self):
        if self.coord_type == "linear":
            ...
            valx = tf.linspace(0.0, np.pi, self._sg.W)[None, None, None, :, None, None]
            valy = tf.linspace(0.0, np.pi, self._sg.H)[None, None, :, None, None, None]
    ...
    def _get_monitored_scalars(self, out_dist, data):
        ...
        with tf.name_scope("monitored_scalars"):
            # ######### Loss Monitoring #########
            scalars["loss/mse"] = tf.losses.mean_squared_error(img, out_dist.mean())
            ...
            pred_mask = self._sg.guard(
                out_dist.mixture_distribution.probs, "B, 1, H, W, K"
            )
