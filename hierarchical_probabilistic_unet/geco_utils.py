...
class MovingAverage(snt.AbstractModule):
  ...
  """
    ...
    def __init__(
        self, decay, local=True, differentiable=False, name="snt_moving_average"
    ):
        super(MovingAverage, self).__init__(name=name)
        self._differentiable = differentiable
        self._moving_average = snt.MovingAverage(decay=decay, local=local, name=name)
...
class LagrangeMultiplier(snt.AbstractModule):
    ...
    def __init__(self, rate=1e-2, name="snt_lagrange_multiplier"):
...
def _sample_gumbel(shape, eps=1e-20):
    ...
    return -tf.log(-tf.log(tf.random_uniform(shape, minval=0, maxval=1) + eps) + eps)
...
def _topk_mask(score, k):
    ...
    return tf.scatter_nd(
        tf.expand_dims(indices, -1), tf.ones(k), tf.squeeze(score).shape.as_list()
    )
