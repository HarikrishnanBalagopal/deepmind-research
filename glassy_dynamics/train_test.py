...
class TrainTest(tf.test.TestCase):
    ...
    def test_load_data(self):
        file_pattern = os.path.join(
            os.path.dirname(__file__), "testdata", "test_small.pickle"
        )
...
class TensorflowTrainTest(tf.test.TestCase):
    ...
    def test_apply_model(self):
        ...
        checkpoint_path = os.path.join(
            os.path.dirname(__file__), "checkpoints", "t044_s09.ckpt"
        )
        file_pattern = os.path.join(
            os.path.dirname(__file__), "testdata", "test_large.pickle"
        )
        predictions = train.apply_model(
            checkpoint_path=checkpoint_path, file_pattern=file_pattern, time_index=0
        )
        data = train.load_data(file_pattern, 0)
