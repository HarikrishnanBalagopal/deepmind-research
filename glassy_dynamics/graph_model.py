...
def make_graph_from_static_structure(positions, types, box, edge_threshold):
  ...
  """
    ...
    cross_positions += tf.cast(cross_positions < -box_ / 2.0, tf.float32) * box_
    cross_positions -= tf.cast(cross_positions > box_ / 2.0, tf.float32) * box_
...
class GraphBasedModel(snt.AbstractModule):
  ...
  """
    ...
    def __init__(self, n_recurrences, mlp_sizes, mlp_kwargs=None, name="Graph"):
    ...
    """
        ...
        model_fn = functools.partial(
            snt.nets.MLP, output_sizes=mlp_sizes, activate_final=True, **mlp_kwargs
        )
        ...
        with self._enter_variable_scope():
            self._encoder = gn_modules.GraphIndependent(
                node_model_fn=model_fn, edge_model_fn=model_fn
            )
            ...
            self._decoder = gn_modules.GraphIndependent(
                node_model_fn=final_model_fn, edge_model_fn=model_fn
            )
