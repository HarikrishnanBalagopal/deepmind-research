...
LossCollection = collections.namedtuple(
    "LossCollection", "l1_loss, l2_loss, correlation"
)
GlassSimulationData = collections.namedtuple(
    "GlassSimulationData", "positions, targets, types, box"
)
...
def get_targets(initial_positions, trajectory_target_positions):
  ...
  """
    targets = np.mean(
        [
            np.linalg.norm(t - initial_positions, axis=-1)
...
def get_loss_ops(prediction, target, types):
  ...
  """
    ...
    return LossCollection(
        ...
        correlation=tf.squeeze(
            tfp.stats.correlation(prediction[:, tf.newaxis], target[:, tf.newaxis])
        ),
    )


def get_minimize_op(loss, learning_rate, grad_clip=None):
...
def _log_stats_and_return_mean_correlation(label, stats):
...
def train_model(
  ...
  """
    if mlp_kwargs is None:
        mlp_kwargs = dict(
            initializers=dict(
                w=tf.variance_scaling_initializer(1.0),
                b=tf.variance_scaling_initializer(0.1),
            )
        )
    # Loads train and test dataset.
    dataset_kwargs = dict(time_index=time_index, max_files_to_load=max_files_to_load)
    ...
    def _make_graph_from_static_structure(static_structure):
        ...
        return (
            graph_model.make_graph_from_static_structure(
    ...
    model = graph_model.GraphBasedModel(n_recurrences, mlp_sizes, mlp_kwargs)
    ...
    with tf.train.SingularMonitoredSession() as session:
        ...
        feed_dict = {
            p: [x[i] for x in training_data] for i, p in enumerate(placeholders)
        }
        session.run(train_iterator.initializer, feed_dict=feed_dict)
        feed_dict = {p: [x[i] for x in test_data] for i, p in enumerate(placeholders)}
...
def apply_model(checkpoint_path, file_pattern, max_files_to_load=None, time_index=9):
  ...
  """
    ...
    with tf.Session() as session:
        ...
        for _ in range(len(data)):
            p, c = session.run(
                (prediction_tensor, correlation_tensor),
    ...
    logging.info(
        "Correlation: %.4f +/- %.4f", np.mean(correlations), np.std(correlations)
    )
    return predictions
