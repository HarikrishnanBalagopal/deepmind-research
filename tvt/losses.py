...
def reconstruction_losses(recons, targets, image_cost, action_cost, reward_cost):
    ...
    if reward_cost > 0.0 and recons.last_reward is not tuple():
        ...
        reward_loss = (
            0.5 * reward_cost * tf.square(recon_last_reward - targets.last_reward)
        )
        reward_loss = sum_time_average_batch(reward_loss)
    else:
        reward_loss = tf.constant(0.0)
...
def read_regularization_loss(
    ...
    if (strength_cost <= 0.0) and (key_norm_cost <= 0.0):
    ...
    if read_info == tuple():
        raise ValueError(
            "Make sure read regularization costs are zero when "
            "not outputting read info."
        )

    read_reg_loss = tf.constant(0.0)
    if strength_cost > 0.0:
    ...
    if strength_cost > 0.0:
        ...
        strength_loss = tf.constant(0.0)

    if key_norm_cost > 0.0:
        key_norm_loss = sum_time_average_batch(key_norm_loss)
    else:
        key_norm_loss = tf.constant(0.0)
