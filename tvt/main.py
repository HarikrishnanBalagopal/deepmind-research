...
flags.DEFINE_integer(
    "logging_frequency", 1, "Log training progress every logging_frequency episodes."
)
flags.DEFINE_string("logdir", None, "Directory for tensorboard logging.")

flags.DEFINE_boolean("with_memory", True, "whether or not agent has external memory.")
flags.DEFINE_boolean(
    "with_reconstruction", True, "whether or not agent reconstruct the observation."
)
flags.DEFINE_float("gamma", 0.92, "Agent discount factor")
flags.DEFINE_float("entropy_cost", 0.05, "weight of the entropy loss")
flags.DEFINE_float("image_cost_weight", 50.0, "image recon cost weight.")
flags.DEFINE_float(
    "read_strength_cost", 5e-5, "Cost weight of the memory read strength."
)
flags.DEFINE_float(
    "read_strength_tolerance",
    2.0,
...
flags.DEFINE_integer(
    "num_episodes", None, "Number of episodes to train for. None means run forever."
)

flags.DEFINE_integer("batch_size", 16, "Batch size")

flags.DEFINE_float("learning_rate", 2e-4, "Adam optimizer learning rate")
flags.DEFINE_float("beta1", 0.0, "Adam optimizer beta1")
...
flags.DEFINE_integer(
    "pycolab_num_apples", 10, "Number of apples to sample from the distractor grid."
)
flags.DEFINE_float(
...
flags.DEFINE_float("pycolab_final_reward", 10.0, "Reward obtained at the last phase.")
flags.DEFINE_boolean("pycolab_crop", True, "Whether to crop observations or not.")
...
def main(_):
    ...
    env_kwargs = {
        ...
        "apple_reward": [
            FLAGS.pycolab_apple_reward_min,
    ...
    reward_ph = tf.placeholder(dtype=tf.float32, shape=batch_shape, name="reward")
    ...
    observations_ph = tf.placeholder(
        dtype=tf.uint8,
        shape=(ep_length + 1, batch_size) + env.observation_shape,
        name="observations",
    )
    ...
    while run:
        ...
        # Compute TVT rewards.
        if FLAGS.do_tvt:
            tvt_rewards = tvt_module.compute_tvt_rewards(
                read_infos, baselines, gamma=FLAGS.gamma
            )
        else:
        ...
        ep_loss, _, ep_loss_logs = sess.run(
            [loss, update_op, loss_logs], feed_dict=loss_feed
        )
        ...
        if ep_num % FLAGS.logging_frequency == 0:
            ...
            logging.info(
                "Episode %d. Last phase reward: %f", ep_num, mean_last_phase_reward
            )
            logging.info(
                "Episode %d. Mean TVT episode reward: %f", ep_num, mean_tvt_reward
            )
            ...
            if FLAGS.logdir:
                ...
                summary.value.add(
                    tag="last phase reward", simple_value=mean_last_phase_reward
                )
