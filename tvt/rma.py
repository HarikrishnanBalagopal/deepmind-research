...
StepOutput = collections.namedtuple("StepOutput", ["action", "baseline", "read_info"])

AgentState = collections.namedtuple("AgentState", ["core_state", "prev_action"])
...
RNNState = collections.namedtuple(
    "RNNState", list(RNNStateNoMem._fields) + ["memory", "mem_reads", "h_mem_writer"]
)
...
def unroll(core, initial_state, inputs, dtype=tf.float32):
    ...
    static_outputs, _ = tf.nn.static_rnn(
        core, inputs=static_rnn_inputs, initial_state=initial_state, dtype=dtype
    )
...
class ImageEncoderDecoder(snt.AbstractModule):
    ...
    def __init__(self, image_code_size, name="image_encoder_decoder"):
    ...
    def decode(self, code):
        ...
        transpose_convnet_in_flat = snt.Linear(
            self._convnet_output_shape.num_elements(), name="decode_initial_linear"
        )(code)
        transpose_convnet_in_flat = tf.nn.relu(transpose_convnet_in_flat)
        transpose_convnet_in = snt.BatchReshape(self._convnet_output_shape.as_list())(
            transpose_convnet_in_flat
        )
...
class Policy(snt.AbstractModule):
    ...
    def _build(self, inputs):
        ...
        baseline = snt.nets.MLP(
            self._baseline_layers, activation=self._activation, name="baseline_mlp"
        )(baseline_in)
        ...
        outputs = PolicyOutputs(policy=policy, action=action, baseline=baseline)
...
class _RMACore(snt.RNNCore):
    ...
    def __init__(self, num_actions, with_memory=True, name="rma_core"):
        ...
        with self._enter_variable_scope():
            ...
            rnn_cores = [
                snt.LSTM(control_hidden_size) for _ in range(control_num_layers)
            ]
        ...
        self._state_size = RNNStateNoMem(
            controller_outputs=controller_out_size, h_controller=controller_state_size
        )
        read_info_size = ()
        if self._with_memory:
            mem_reads_size, read_info_size = self._memory_reader.output_size
            mem_writer_state_size = self._memory_writer.state_size
            self._state_size = RNNState(
                memory=tf.TensorShape(self._mem_shape),
    ...
    def _build(self, inputs, h_prev):
        ...
        if self._with_memory:
            ...
            mem_reads, read_info = self._memory_reader((controller_out, h_prev.memory))
            memory, h_mem_writer = self._memory_writer(
                (z, h_prev.memory), h_prev.h_mem_writer
            )
        ...
        core_outputs = CoreOutputs(z=z, read_info=read_info, **policy_outputs._asdict())

        h_next = RNNStateNoMem(
            controller_outputs=controller_out, h_controller=h_controller
        )
        if self._with_memory:
            h_next = RNNState(
                memory=memory,
  ...
  """
    ...
    def __init__(
        ...
        with self._enter_variable_scope():
            ...
            self._core = _RMACore(
                num_actions=self._num_actions, with_memory=with_memory
            )
    ...
    def _prepare_observations(self, observation, last_reward, last_action):
        ...
        return Observation(
            image=image, last_action=last_action, last_reward=last_reward
        )
    ...
    def _decode(self, z):
        ...
        recons = Observation(
            image=image_recon, last_reward=reward_recon, last_action=action_recon
        )
    ...
    def step(self, reward, observation, prev_state):
        ...
        agent_state = AgentState(core_state=next_core_state, prev_action=action)
    ...
    def loss(self, observations, rewards, actions, additional_rewards=None):
        ...
        agent_loss_log = losses.combine_logged_values(
            a2c_loss_logged_values, recon_logged_values, read_reg_logged_values
        )
