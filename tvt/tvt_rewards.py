...
def _tvt_rewards_single_head(
  ...
  """
    ...
    read_weights_masked = np.where(
        read_past_no_transport_period, read_weights, np.zeros_like(read_weights)
    )
    ...
    times = np.arange(ep_length)
    max_read_weight_times = read_times[times, max_read_weight_args]  # (ep_length,)
    ...
    for t2 in t2_times_with_largest_reads:
        try:
            ...
            raise RuntimeError(
                "Attempting to access baselines array with length {}"
...
def compute_tvt_rewards(read_infos, baselines, gamma=0.96):
