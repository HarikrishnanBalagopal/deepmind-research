...
class MemoryReader(snt.AbstractModule):
    ...
    """
        ...
        # Read from memory.
        memory_reads, read_weights, read_indices, read_strengths = read_from_memory(
            read_keys, read_strengths, mem_state, self._top_k
        )
...
def read_from_memory(read_keys, read_strengths, mem_state, top_k):
  ...
  """
    _assert_compatible_read_from_memory_inputs(read_keys, read_strengths, mem_state)
    ...
    with tf.name_scope("memory_reading"):
        ...
        cosine_distances = tf.matmul(
            scaled_read_keys, scaled_mem, transpose_b=True
        )  # [B, H, R]
        ...
        if top_k:
            ...
            expanded_scaled_read_keys = tf.expand_dims(
                scaled_read_keys, axis=2
            )  # [B, H, 1, M]
            topk_cosine_distances = tf.reduce_sum(
                expanded_scaled_read_keys * topk_scaled_mem, axis=-1
            )  # [B, H, K]
            topk_weighted_distances = (
                read_strengths * topk_cosine_distances
            )  # [B, H, K]
            read_weights = tf.nn.softmax(topk_weighted_distances, axis=-1)  # [B, H, K]
            ...
            memory_reads = tf.reduce_sum(
                expanded_weights * topk_mem, axis=2
            )  # [B, H, M]
        else:
...
def _assert_compatible_read_from_memory_inputs(read_keys, read_strengths, mem_state):
    ...
    read_strengths.shape.assert_is_compatible_with(tf.TensorShape([b_shape, h_shape]))
