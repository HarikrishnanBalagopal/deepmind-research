...
class TransporterTest(tf.test.TestCase, parameterized.TestCase):
    @parameterized.parameters(
        {"norm_type": "batch"}, {"norm_type": "layer"}, {"norm_type": "instance"}
    )
    def test_output_shape(self, norm_type):
        ...
        model = transporter.Transporter(
            encoder=encoder, decoder=decoder, keypointer=keypointer
        )
    ...
    """
        ...
        encoder = transporter.Encoder(
            filters=FILTERS, strides=STRIDES, kernel_sizes=KERNEL_SIZES
        )
        ...
        keypoint_encoder = transporter.Encoder(
            filters=FILTERS[:-2], strides=STRIDES[:-2], kernel_sizes=KERNEL_SIZES[:-2]
        )
...
class EncoderTest(tf.test.TestCase):
    def test_output_shape(self):
        ...
        self.assertEqual(
            features.shape, (BATCH_SIZE, IMAGE_H // 4, IMAGE_W // 4, filters[-1])
        )
...
class KeyPointerTest(tf.test.TestCase):
    def test_output_shape(self):
        ...
        self.assertEqual(
            keypointer_results["centers"].shape, (BATCH_SIZE, num_keypoints, 2)
        )
        self.assertEqual(
            keypointer_results["heatmaps"].shape,
...
class DecoderTest(tf.test.TestCase):
    def test_output_shape(self):
        feature_batch = tf.random.normal(
            shape=(BATCH_SIZE, IMAGE_H // 4, IMAGE_W // 4, 64)
        )

        decoder = transporter.Decoder(
            initial_filters=64, output_size=[IMAGE_H, IMAGE_W], output_channels=IMAGE_C
        )
    ...
    def test_encoder_decoder_output_shape(self):
        ...
        decoder = transporter.Decoder(
            initial_filters=4, output_size=[IMAGE_H, IMAGE_W], output_channels=IMAGE_C
        )
